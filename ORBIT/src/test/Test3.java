...
public class Test3 {
	public static void main2(String arg[]) {
		...
		electionTest(numRaces, numCandidates, numVotes, miners, rand, pub, bitSeparation);
	}

	public static void electionTest(int numRaces, int numCandidates, int numVotes, int miners, SecureRandom rand,
		...
		BallotTransaction[] ballots = Test3.createTransactions(election, encryptedVotes, blockchain, rand);

		Thread[] minerThreads = new Thread[miners];
...
//					out[0][i].writeObject(minerKey);
	...
	public static BallotTransaction[] createTransactions(Election election, EncryptedVote[][] encryptedVotes, ProcessedBlockchain blockchain, SecureRandom rand) {
		int ringSize = 15;
		ECNamedCurveParameterSpec spec = ECNamedCurveTable.getParameterSpec("secp256k1");
		...
		System.out.println("Verifying Ballot Transaction");
		for(int i = 0; i < ballots2.length; i++) {
