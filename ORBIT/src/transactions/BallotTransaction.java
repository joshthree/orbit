...
public class BallotTransaction implements VoterTransaction {
	...
	public BallotTransaction(SourceTransaction[] ringMembers, int source, Additive_Priv_Key signingKey, Additive_Priv_Key newKey, BigInteger passwordGuess, BigInteger passwordDisplacement, ElectionTransaction electionTx, EncryptedVote[] votes, BigInteger passwordRandomization, SecureRandom rand){
	...
	public boolean minerProcessBallot(ProcessedBlockchain blockchain, AdditiveElgamalPrivKey minerPrivKey, AdditiveElgamalPubKey[] individualMinerKeys, ObjectInputStream[] in, ObjectOutputStream[] out, SecureRandom rand) {
		...
		for(int i = 0; i < rowCount/2; i++) {
			table1[i][0] = minerKey.encrypt(BigInteger.ONE, BigInteger.ZERO);
			table1[i][1] = minerKey.encrypt(BigInteger.ONE, BigInteger.ZERO);
		}
		for(int i = rowCount/2; i < rowCount; i++) {
			table1[i][0] = minerKey.encrypt(BigInteger.TWO, BigInteger.ZERO);
			table1[i][1] = minerKey.encrypt(BigInteger.ONE, BigInteger.ZERO);
		}
		boolean retry = false;
		do {
		...
		}
		{
			...
			dummyTable0[0] = new ElectionTableRowInner(minerKey.encrypt(BigInteger.ZERO, BigInteger.ZERO), voterVoteWithoutProof, null);
			dummyTable0[1] = new ElectionTableRowInner(minerKey.encrypt(BigInteger.ONE, BigInteger.ZERO), zeroVote, null);
			int[] order = MinerThread.chooseOrder(in, out, minerKey, rand);
			for(int i = 0; i < in.length; i++) {
				...
				if(in[order[i]] == null) { //my turn
					...
					if(swap) {
						intermediateDummyTables[i][0] = orig[1].rerandomize(compareRerandmize0, raceRerandomizer[0], null, election);
						intermediateDummyTables[i][1] = orig[0].rerandomize(compareRerandmize1, raceRerandomizer[1], null, election);
					} else {
						intermediateDummyTables[i][0] = orig[0].rerandomize(compareRerandmize0, raceRerandomizer[0], null, election);
						intermediateDummyTables[i][1] = orig[1].rerandomize(compareRerandmize1, raceRerandomizer[1], null, election);
					}
					...
					CryptoData[] fullProofData = getSmallTableProverData(rand, election, minerKey, i, orig, swap,
							compareRerandmize0, compareRerandmize1, raceRerandomizer, intermediateDummyTables, null);
		...
		EncryptedVote[] dummyTableOutput = null;
		...
		if(dummyInputsDecrypted[0].getCipher(minerKey).equals(curve.getInfinity())) {
			if(dummyInputsDecrypted[1].getCipher(minerKey).equals(curve.getInfinity())) {
				...
				dummyTableOutput = intermediateDummyTables[in.length-1][0].output1;
			...
			dummyTableOutput = intermediateDummyTables[in.length-1][1].output1;
		...
		passwordTable[0] = new ElectionTableRowInner(minerKey.getEmptyCiphertext().homomorphicAdd(passwordDifferenceCipher.negate(minerKey), minerKey), dummyTableOutput, dummyFlagOutput);
...
//		private CryptoData[][] dummyProofTranscripts;
	...
	public CryptoData[] getSmallTableVerifierData(Election election, AdditiveElgamalPubKey minerKey, int i,
		...
		boolean third = (orig[0].output2 != null);
...
//				System.out.println(orig[0].output1[j]);
			...
			midPub[1] = new CryptoDataArray(inner2Pub);
			midEnv[1] = new CryptoDataArray(inner2Env);
			...
			if(third) {
				...
				midPub[2] = new CryptoDataArray(mid3Pub);
				midEnv[2] = new CryptoDataArray(mid3Env);
			}
	...
	public CryptoData[] getSmallTableProverData(SecureRandom rand, Election election, AdditiveElgamalPubKey minerKey,
		...
		{//Build non-swap half of data
			...
			midPub[1] = new CryptoDataArray(inner2Pub);
			midSec[1] = new CryptoDataArray(inner2Sec);
			midEnv[1] = new CryptoDataArray(inner2Env);
			...
			CryptoData[][] mid3 = new CryptoData[2][]; 
				...
				midPub[2] = new CryptoDataArray(mid3Pub);
				midSec[2] = new CryptoDataArray(mid3Sec);
				midEnv[2] = new CryptoDataArray(mid3Env);
			}
