...
//				System.out.println("gur8wu984ugoierj MAIN EPHEMERAL SET TO 0");;
//				mainEphemerals[k][k2] = BigInteger.ZERO;
				table2s[1][k][k2] = table2s[0][mainShuffle[k]][k2].rerandomize(mainEphemerals[k][k2], minerKey);
...
//					ephemerals[j][0][k][k2] = BigInteger.ZERO;
					otherTables[j][k][k2] = sourceOtherTables[j][shuffle[j][0][k]][k2].rerandomize(ephemerals[j][0][k][k2], minerKey);
...
//		}
		...
		for(int i = 0; i < numTrials; i++) {
			...
			if(challenge.testBit(i) || true) {
				...
				for(int j = 0; j < rows; j++) {
					System.out.println("gdsigyerighdkfsjglkfb " + pF.getVector()[j]);
					for(int k = 0; k < cols; k++) { 
						newEphemerals[j][k] = oldEphemerals[j][k].add(ephemerals[i][0][newShuffleInv[j]][k]).subtract(mainEphemerals[j][k]).mod(curve.getOrder());
//						newEphemerals[newShuffle[j]][k] = oldEphemerals[pF.getVector()[j]][k].add(ephemerals[i][0][p0.rightMultiply(pF).getVector()[j]][k]).subtract(mainEphemerals[pF.getVector()[j]][k]).mod(curve.getOrder());

//						newEphemerals[newShuffle[j]][k] = oldEphemerals[base[j]][k].add(ephemerals[i][0][pF.getVector()[base[j]]][k]).subtract(mainEphemerals[pF.getVector()[base[j]]][k]).mod(curve.getOrder());
//						newEphemerals[j][k] = oldEphemerals[j][k].add(ephemerals[i][0][j][k]).subtract(mainEphemerals[j][k]).mod(curve.getOrder());
					} //oldEphemerals has no effect at 1 miner.
				}
				BigInteger[][] TEMPOTHEREPHEMERALS = null;
				...
				if(myPos != order.length-1) {
					...
					try {
						...
						TEMPOTHEREPHEMERALS = (BigInteger[][]) in[order[order.length-1]].readObject();
					...
					for(int k = 0; k < order.length-1; k++) {
						try {
							System.out.println("GAJHFIDAUHGFDKJSGFDSJK TEMP SENDING INFO");
							BigInteger[][] ephemeralsTemp = new BigInteger[rows][];
							BigInteger[][] mainEphemeralsTemp = new BigInteger[rows][];
							
							for(int j = 0; j < ephemeralsTemp.length; j++) {
								ephemeralsTemp[j] = ephemerals[i][0][p0.rightMultiply(pF).getVector()[j]];
							}

							for(int j = 0; j < ephemeralsTemp.length; j++) {
								mainEphemeralsTemp[j] = mainEphemerals[pF.getVector()[j]];
							}
							out[order[k]].writeObject(ephemeralsTemp);
							out[order[k]].writeObject(mainEphemeralsTemp);
				...
				for(int j = 0; j < rows; j++) {
					System.out.println(Thread.currentThread() + " 48tugkfdjhgoi " + sourceTable[netShuffle[j]][0].rerandomize(newEphemerals[j][0], minerKey).getCipher(minerKey).equals(destTable[j][0].getCipher(minerKey)));
...
//								int j4 = j;
//								if(sourceTable[j][0].rerandomize(ephemerals[i][0][pF.getVector()[otherPF[j0]]][0].subtract(mainEphemerals[pF.getVector()[otherPF[j1]]][0]).add(TEMPOTHEREPHEMERALS[j2][0]).subtract(TEMPOTHERMAINEPHEMERALS[j3][0]).mod(curve.getOrder()), minerKey).getCipher(minerKey).equals(destTable[netShuffle[j4]][0].getCipher(minerKey))) {
//									System.out.println("57987298gjfdskj " + true + " " + j + " " + j0 + " " + j1 + " " + j2 + " " + j3 + " " + j4);
//								}
//							}
//						}
////						}
