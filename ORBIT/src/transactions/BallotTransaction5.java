...
public class BallotTransaction5 implements BallotT {
	...
	public BallotTransaction5(SourceTransaction[] ringMembers, int source, Additive_Priv_Key signingKey, Additive_Priv_Key newKey, BigInteger passwordGuess, BigInteger passwordDisplacement, ElectionTransaction electionTx, EncryptedVote[] votes, BigInteger passwordRandomization, SecureRandom rand){
		super();
	...
	public boolean minerProcessBallot(ProcessedBlockchain blockchain, AdditiveElgamalPrivKey minerPrivKey, AdditiveElgamalPubKey[] individualMinerKeys, ObjectInputStream[] in, ObjectOutputStream[] out, SecureRandom rand) {
		...
		try {
			...
			ECPoint a1 = curve.getInfinity();
			ECPoint a2 = curve.getInfinity();
			for(int i = party-1; (i  + in.length) % in.length!= party; i--) {
				proofEphemerals[(i + in.length) % in.length] = minerKey.generateEphemeral(rand);
				ECPoint eph = current.getEphemeral(minerKey);
				a1 = a1.add(g.multiply(proofEphemerals[(i + in.length) % in.length]));
				a2 = a2.add(eph.multiply(proofEphemerals[(i + in.length) % in.length]));
				current = minerPrivKey.decrypt(current);
				out[(party+1)%in.length].writeObject(current);
				out[(party+1)%in.length].writeObject(a1.getEncoded(true));
				out[(party+1)%in.length].writeObject(a2.getEncoded(true));
				out[(party+1)%in.length].flush();
				current = (AdditiveElgamalCiphertext) in[(party + in.length - 1) % in.length].readObject();
				a1 = curve.decodePoint((byte[]) in[(party + in.length - 1) % in.length].readObject());
				a2 = curve.decodePoint((byte[]) in[(party + in.length - 1) % in.length].readObject());
			}
			AdditiveElgamalCiphertext current2 = current;
			current = minerPrivKey.decrypt(current);
			ECPoint eph = current.getEphemeral(minerKey);
			...
			CryptoData[] weirdProofProverData = new CryptoData[3];
			{
				...
				pubData[1] = new ECPointData(combinedCiphers[party].getCipher(minerKey));
				int counter = 2;
				for(int i = 0; i < voterVotes.length; i++) {
				...
				envData[1] = new ECPointData(combinedCiphers[party].getEphemeral(minerKey));
				...
				unaltered[0] = new ECPointData(unaltered[0].getECPointData(curve).add(a1));
				unaltered[1] = new ECPointData(unaltered[1].getECPointData(curve).add(a2));
				...
				for(int i = 1; i < in.length; i++) {
					out[(i+party)%in.length].writeObject(fiatChallenge);
				}
				for(int i = 1; i < in.length; i++) {
					out[(i+party)%in.length].flush();
				}
				for(int i = in.length-1; i >= 1; i--) {
					BigInteger otherChallenge = (BigInteger) in[(i+party)%in.length].readObject();
					BigInteger z = (minerPrivKey.getPrivKey()[0].multiply(otherChallenge).add(proofEphemerals[(i+party)%in.length])).mod(curve.getOrder());
					out[(i+party)%in.length].writeObject(z);
				}
				for(int i = in.length-1; i >= 1; i--) {
					out[(i+party)%in.length].flush();
				}
				BigInteger otherZ = BigInteger.ZERO;

				for(int i = 1; i < in.length; i++) {
					otherZ = otherZ.add((BigInteger) in[(i+party)%in.length].readObject()).mod(curve.getOrder());
				}
				...
				unaltered[1] = new BigIntData(unaltered[1].getBigInt().add(otherZ).mod(curve.getOrder()));
				...
				for(int j = 0; j < myVersion.length; j++) {
					...
					myVersion[j] = myVersion[j].scalarMultiply(BigInteger.valueOf(5).modInverse(raceKey.getOrder()), raceKey);
				}
			}
